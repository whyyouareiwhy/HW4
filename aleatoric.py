...
def sin_wave(freq, amp, clip):
    ...
    # root note amplitude = --accent and non-root notes = --volume
    audio *= (amp/10)

    # Square the wave on the root note if clip arg is True
    if clip:
        audio = np.clip(audio, a_min=-8192, a_max=8192)
        wav.write('clipped.wav', SAMPLERATE, audio)
...
def square_wave(freq, amp):
    ...
    # audio = scipy.signal.square(2 * np.pi * SAMPLERATE * t)
    # audio = audio.astype(np.int16)
    return t
...
def play_measure(octave_notes):
    for note in major_scale_notes:
        ...
        audio_sample = sin_wave(pitch, options.accent)
...
def random_music(measure):
    count = 1
    '''TO-DO - square the root-note wave'''
    # Play root note with --accent arg settings
    root = major_notes[0]
    print(f"{root} / {format(measure[root], '.2f')} / beat {count}")
    count += 1
    root_note = sin_wave(measure[root], options.accent, True)
    wave_obj = sa.WaveObject(root_note, 1, 2, SAMPLERATE)
    play_obj = wave_obj.play()
    play_obj.wait_done()

    # play (--beats - 1) random notes from major scale
    for i in range(1, options.beats):
        rand_note = random.choice(major_notes)
        print(f"{rand_note} / {format(measure[rand_note], '.2f')} / beat {count}")
        count += 1
        non_root_note = sin_wave(measure[rand_note], options.volume, False)
...
octave = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B', '_C']  # C_ is next octave's C
major_scale = [0, 2, 4, 5, 7, 9, 11, 12]  # major scale ends on root C note of next octave
...
print(f"--root {options.root} --beats {options.beats} --bpm {options.bpm} --ramp {options.ramp} --accent {options.accent} --volume {options.volume}")
while True:
    if options.root == 36:
